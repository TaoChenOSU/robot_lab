...
def train_net(data, model, loss_func, optimizer, batch_size, max_epochs,
	...
	for epoch in xrange(max_epochs):
		...
		for iter in xrange(iter_start, iter_end):
			...
			#############################################################################
			forward_result = model.forward(data_batch)
			loss = loss_func.forward(forward_result, labels_batch)
		...
		#############################################################################
		training_result = np.argmax(model.forward(data_train), axis=1)
		train_acc = np.mean(training_result == labels_train)
		validation_result = np.argmax(model.forward(data_val), axis=1)
		val_acc = np.mean(validation_result == labels_val)
		#############################################################################
		#                             END OF YOUR CODE                              #
		#############################################################################
