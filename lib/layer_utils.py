...
class fc(object):
	...
	def forward(self, feat):
		...
		#############################################################################
		flatten_feat = feat.reshape((feat.size/self.input_dim, self.input_dim))
		output = flatten_feat.dot(self.params[self.w_name]) + self.params[self.b_name]
		#############################################################################
		#                             END OF YOUR CODE                              #
		#############################################################################
		self.meta = feat
...
class dropout(object):
	...
	def forward(self, feat, is_Training=True):
		...
		if not is_Training:
			self.p = 1
		dropped = np.random.binomial(1, self.p, size=np.prod(feat.shape))

		output = np.multiply(feat.flatten(), dropped).reshape(feat.shape)
		#############################################################################
		#                             END OF YOUR CODE                              #
		#############################################################################
	...
	def backward(self, dprev):
		...
		if not self.is_Training:
			...
			dfeat = np.multiply(self.dropped, dprev.flatten()).reshape(dprev.shape)
		#############################################################################
		#                             END OF YOUR CODE                              #
		#############################################################################
		self.is_Training = False
...
class cross_entropy(object):
	...
	def forward(self, feat, label):
		...
		#############################################################################
		assert feat.shape[0] == label.shape[0], "Batch sizes are different"
		batch_size = feat.shape[0]
		num_of_classes = feat.shape[1]
		# create the ground truth mask
		ans_mask = np.zeros([batch_size, num_of_classes])
		ans_mask[range(batch_size), label] = 1.0
		loss = -(1.0/batch_size)*np.sum(ans_mask*np.log(scores))
		#############################################################################
		#                             END OF YOUR CODE                              #
		#############################################################################
	...
	def backward(self):
		...
		dLoss[range(batch_size), self.label] -= 1.0
...
def softmax(feat):
	...
	num_of_classes = feat.shape[1]
	scores = np.exp(feat)
	scores = scores/scores.dot(np.ones((num_of_classes, 1)))
	#############################################################################
	#                             END OF YOUR CODE                              #
	#############################################################################
	return scores
